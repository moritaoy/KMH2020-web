package com.kmh2020.demo.logic;
...
public class RequestLogic {
	...
	public Model meeting(Model model, String meetingId) {
		model.addAttribute("request_meetingId", meetingId);
		
		OkHttpClient okHttpClient = new OkHttpClient();
		MediaType MIMEType = MediaType.parse("application/json; charset=utf-8");
		String api_url = "https://gdnw3shrsk.execute-api.ap-northeast-1.amazonaws.com/dev/meeting?meetingId=";
		
		ObjectMapper res_mapper = new ObjectMapper();
		ObjectNode res_node = res_mapper.createObjectNode();

		Request request = new Request.Builder().url(api_url + meetingId).get().build();
		Call call = okHttpClient.newCall(request);
		String responce_str="";
		List<String> sessionList = new ArrayList<String>();
		try (Response response = call.execute()) {
			if (response.body() == null) {
				res_node.put("msg_warning", "サーバーからデータを受信できませんでした");
				return model;
			}
			responce_str = response.body().string();
			...
			sessionList = jsonMapper.readValue(node.get("data").textValue(), new TypeReference<List<String>>() {});
		} catch (IOException e) {
			e.printStackTrace();
		}
		String api_session = "https://gdnw3shrsk.execute-api.ap-northeast-1.amazonaws.com/dev/user/data/?sessionId=";
		for (int i = 0; i < sessionList.size(); i++) {
			String target = sessionList.get(i);
			if(target=="")continue;
			request = new Request.Builder().url(api_url + target).get().build();
